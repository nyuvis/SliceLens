<script lang='ts'>
  ...
  $: queryResults = getViolinPlotsData(xCol).then(({data, maxDensity}) => {
...
<svg>
  {#await queryResults then result}
    {#each result.data as {cat, min, max, q1, q2, q3, iqr, densities}}
      <g transform='translate({x(cat) + x.bandwidth() / 2})'>
        <!-- KDE -->
        <path
          d={result.area(densities)}
          fill='#069b62'
          fill-opacity={0.25}
        />
