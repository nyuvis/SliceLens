<script lang='ts'>
  ...
  import { getStackedHistogramData } from '../../../lib/VisQueries';
  ...
  const numBins = 20;
  const binSize = (xDomain[1] - xDomain[0]) / numBins;

  $: barWidth = x(xDomain[0] + binSize) - x(xDomain[0]);
  ...
  $: queryResults = getStackedHistogramData(xCol, binSize, xDomain[0]);
